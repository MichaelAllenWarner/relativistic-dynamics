...
\end{aligned}
\end{equation}
We've used Equations \ref{eq:fc}, \ref{eq:fp}, \ref{eq:eb}, \ref{eq:ace}, and \ref{eq:abe}, and a pair of vector calculus identities (the divergence of a curl is $0$, and the curl of a gradient is $\vv 0$). 
...
\subsubsection{An Aside on Momentum and Euclidean Rank-2 Tensors}\label{sssec:emm}
...
\end{equation*}
A comparison with Poynting's theorem (Equation \ref{eq:py}) shows why the Poynting vector is sometimes called the \textbf{electromagnetic momentum density} (it represents both that \emph{and} the flow of electromagnetic energy). Don't worry about the sign difference in the divergence terms---the volume-integral of $- \del \cdot \vvsigma$ is indeed the \emph{outward} flux of electromagnetic three-momentum from a region.
...
\begin{equation*}
\begin{aligned}
\textrm{\mbox{\boldmath $\emptyset$}} &= \Box \left( \bar{\vv A} \mathrm{e}^{\mathrm{i} f} \right) = \partialup \cdot \partialup \left( \bar{\vv A} \mathrm{e}^{\mathrm{i} f} \right) \\
0 &= \partialup \cdot \left[ \mathrm{e}^{\mathrm{i} f} \, \partialup ( \mathrm{i} f ) \right] \\
...
\end{aligned}
\end{equation*}
and our constraints become $\partialup \cdot \vv K = 0$ and $\vv K \cdot \vv K = 0$. So $\vv K$ \emph{is} lightlike, and let's further insist that it's future-pointing (otherwise we allow for light waves traveling backward in time!). The Lorenz condition ($\partialup \cdot \vv A = 0$) imposes an additional constraint on $\vv K$:
\begin{equation*}
\begin{aligned}
